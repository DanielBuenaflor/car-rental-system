...
def api_upload_vehicle_images(vehicle_id):
    ...
    if 'primary_image' in request.files:
        ...
        if file and file.filename:
            filename, error = save_upload(file, UPLOAD_FOLDER, allowed_extensions=ALLOWED_IMAGE_EXTENSIONS)
            if error:
...
def api_update_booking(booking_id):
    ...
    try:
        ...
        cursor.execute("""
            SELECT user_id, booking_reference, b.brand_name, b.model
            FROM bookings b
            JOIN vehicles v ON b.vehicle_id = v.id
            JOIN vehicle_brands vb ON v.brand_id = vb.id
            WHERE b.id = %s
        ...
        if booking and cursor.rowcount > 0:
            user_id = booking['user_id']
            ref = booking['booking_reference']
            vehicle_name = f"{booking['brand_name']} {booking['model']}"

...
if (!window._locationMapInitialized) {
        ...
        document.querySelectorAll('.location-tab').forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                setTimeout(() => {
                    ...
                    if (tabId === 'pickup-custom' && customPickupMap) {
                        customPickupMap.invalidateSize();
                        // Re-center if we have a saved location
                        const lat = document.getElementById('custom_pickup_lat')?.value;
                        const lng = document.getElementById('custom_pickup_lng')?.value;
                        if (lat && lng) customPickupMap.setView([lat, lng], 15);
                    }
                    if (tabId === 'return-custom' && customReturnMap) {
                        customReturnMap.invalidateSize();
                        // Re-center if we have a saved location
                        const lat = document.getElementById('custom_return_lat')?.value;
                        const lng = document.getElementById('custom_return_lng')?.value;
                        if (lat && lng) customReturnMap.setView([lat, lng], 15);
                    }
                }, 150);
            });
        });
    ...
    function setupCustomAddressInput() {
        const customPickupInput = document.getElementById('custom_pickup_street');
        if (customPickupInput) {
            customPickupInput.addEventListener('input', debounce((e) => {
                const fullAddress = e.target.value ? `${e.target.value}, Laguna, Philippines` : 'Laguna, Philippines';
                geocodeAddress(fullAddress, 'pickup');
                
                document.getElementById('custom_pickup_address').value = e.target.value ? `${e.target.value}, Laguna` : 'Laguna';
            }, 800));
        }
        
        const customReturnInput = document.getElementById('custom_return_street');
        if (customReturnInput) {
            customReturnInput.addEventListener('input', debounce((e) => {
                const fullAddress = e.target.value ? `${e.target.value}, Laguna, Philippines` : 'Laguna, Philippines';
                geocodeAddress(fullAddress, 'return');
                
                document.getElementById('custom_return_address').value = e.target.value ? `${e.target.value}, Laguna` : 'Laguna';
            }, 800));
        }
    }
